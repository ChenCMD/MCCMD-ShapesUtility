execute as @e[type=area_effect_cloud,tag=chenbr_shapes_util-temp] if score @s SummonOrder = #from Core at @s run tp 0-a-17a7-3-0 ~ ~ ~
execute as @e[type=area_effect_cloud,tag=chenbr_shapes_util-temp] if score @s SummonOrder = #to Core at @s run tp 0-a-17a7-3-1 ~ ~ ~
